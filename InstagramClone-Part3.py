...
def displayimages():
    ...
    query = " SELECT classification,d1.File_Name,Image_type,Uploaded_time FROM image_details d1, details d2 where d1.File_Name = d2.File_Name and ingredients  = '" + ingredients  + "'"
...
@app.route('/displaycalories',methods=['POST','GET'])
def displaycalories():
    i = 0
    alldetails = {};
    ...
    query = " SELECT classification,d1.File_Name,Image_type,Uploaded_time FROM image_details d1, details d2 where d1.File_Name = d2.File_Name and quantity  > " +  calories + " group by d1.file_name"
    cursor.execute(query)
    # fetch all of the rows from the query
    data = cursor.fetchall()
    if len(data) > 0:
